---
title: "Nhật ký tuần 8"
date: 2026-06-22
weight: 8
chapter: false
pre: " <b> 1.8. </b> "
---

{{% notice info %}}
📅 **Thời gian:** 22/06/2026 – 28/06/2026
{{% /notice %}}

### Mục tiêu tuần 8

- Hoàn thành nội dung lý thuyết của Module 4.
- Thực hành Lab 13 của Module 4.
- Tìm hiểu quy trình sao lưu và bảo vệ tài nguyên trên AWS.
- Hiểu cách cấu hình thông báo và kiểm tra khả năng khôi phục dữ liệu.

### Công việc thực hiện

| Ngày | Nội dung                                                                                      | Bắt đầu    | Hoàn thành | Tài liệu tham khảo                                                       |
| ---- | --------------------------------------------------------------------------------------------- | ---------- | ---------- | ------------------------------------------------------------------------ |
| 1    | Hoàn thành nội dung lý thuyết Module 4.                                                       | 22/06/2026 | 23/06/2026 | https://www.youtube.com/playlist?list=PLahN4TLWtox2a3vElknwzU_urND8hLn1i |
| 2    | Thực hành **Lab 13**, bao gồm tạo Amazon S3 Bucket và triển khai hạ tầng cần thiết.           | 24/06/2026 | 25/06/2026 | https://www.youtube.com/playlist?list=PLahN4TLWtox2a3vElknwzU_urND8hLn1i |
| 3    | Tạo và cấu hình **AWS Backup Plan**, đồng thời thiết lập các thông báo cho quá trình sao lưu. | 26/06/2026 | 27/06/2026 | https://www.youtube.com/playlist?list=PLahN4TLWtox2a3vElknwzU_urND8hLn1i |
| 4    | Thực hiện **Test Restore** để kiểm tra khả năng khôi phục dữ liệu từ bản sao lưu.             | 28/06/2026 | 28/06/2026 | https://www.youtube.com/playlist?list=PLahN4TLWtox2a3vElknwzU_urND8hLn1i |

### Kết quả đạt được

- Hoàn thành nội dung lý thuyết của Module 4.
- Hoàn thành **Lab 13** của Module 4.
- Tạo và cấu hình thành công **Amazon S3 Bucket** phục vụ cho bài thực hành.
- Triển khai hạ tầng cần thiết cho quá trình sao lưu dữ liệu.
- Tạo **AWS Backup Plan** và thiết lập chính sách sao lưu tự động.
- Cấu hình thông báo để theo dõi trạng thái các tác vụ sao lưu.
- Thực hiện thành công quá trình **Restore**, xác nhận khả năng khôi phục dữ liệu khi cần thiết.
- Củng cố kiến thức về quản lý sao lưu, giám sát và khôi phục dữ liệu nhằm nâng cao khả năng bảo vệ tài nguyên trên nền tảng AWS.
